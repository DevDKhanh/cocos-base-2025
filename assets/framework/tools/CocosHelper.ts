...
export default class CocosHelper {
  ...
  static addSprite(
    ...
    if (typeof options.sizeMode !== "number")
      options.sizeMode = Sprite.SizeMode.TRIMMED;
  ...
  static addLabel(
    ...
    if (typeof options.horizontalAlign !== "number")
      options.horizontalAlign = Label.HorizontalAlign.CENTER;
    if (typeof options.verticalAlign !== "number")
      options.verticalAlign = Label.VerticalAlign.CENTER;
    if (typeof options.overflow !== "number")
      options.overflow = Label.Overflow.NONE;
    if (typeof options.cacheMode !== "number")
      options.cacheMode = Label.CacheMode.NONE;
  ...
  static addWidget(
    nd: Node,
    paddings: { left?: number; right?: number; top?: number; bottom?: number }
  ) {
    if (!nd) return;
  ...
  static addLayout(
    ...
    if (typeof options.resizeMode !== "number")
      options.resizeMode = Layout.ResizeMode.CONTAINER;
    ...
    if (lo.type == Layout.Type.HORIZONTAL) {
      lo.horizontalDirection =
        typeof options.horizontalDirection === "number"
      ...
      lo.verticalDirection =
        typeof options.verticalDirection === "number"
      ...
      lo.startAxis =
        typeof options.startAxis === "number"
          ? options.startAxis
  ...
  static grayNode(node: Node, isGray: boolean = true) {
    if (node.getComponent(Sprite)) {
      ...
      node
        ...
        .setSharedMaterial(
          builtinResMgr.get(
            isGray ? "ui-sprite-gray-material" : "ui-sprite-material"
          ),
          0
        );
    }
...
export const coin = (number: string | number) =>
  Math.floor(Number(number)).toLocaleString("en-GB");

export const padStart = (str: string, length: number, pad: string): string => {
  return str.length >= length
    ? str
    : new Array(length - str.length + 1).join(pad) + str;
};

export const dateFormat = (date: string | number, format: string): string => {
  ...
  const month = padStart((d.getMonth() + 1).toString(), 2, "0");
  const day = padStart(d.getDate().toString(), 2, "0");
  const hours = padStart(d.getHours().toString(), 2, "0");
  const minutes = padStart(d.getMinutes().toString(), 2, "0");
  const seconds = padStart(d.getSeconds().toString(), 2, "0");

  // Tạo đối tượng với các phần tử ngày giờ để thay thế trong format
  const dateParts = {
    YYYY: year,
    MM: month,
    DD: day,
  ...
  // Thay thế các phần tử trong format string bằng các giá trị tương ứng
  return format.replace(/YYYY|MM|DD|HH|mm|ss/g, (match) => dateParts[match]);
};
